...
module.overlap<-function(res.module, show.mods=NULL, type=c("model","max.patients", "max.genes")[1], max.n=30,  show.overlap=TRUE, cex=10){
	...
	if(length(unique(as.vector(olp1)))>1){
		if(show.overlap){
			Heatmap(olp3,  cluster_rows = TRUE, cluster_columns = TRUE,name="Patients",column_title="Patients", cell_fun = function(j, i, x, y, w, h, col) {
		    	grid.text(olp1[i, j], x, y, gp=gpar(fontsize=cex))
			}) +
			Heatmap(olp4, cluster_rows = TRUE, cluster_columns = TRUE,name="Genes", column_title="genes" , cell_fun = function(j, i, x, y, w, h, col) {
		    	grid.text(olp2[i, j], x, y, gp=gpar(fontsize=cex))
			})
		}
		else{
			Heatmap(olp3, cluster_rows = TRUE, cluster_columns = TRUE,name="Patients",column_title="Patients") +
			Heatmap(olp4, cluster_rows = TRUE, cluster_columns = TRUE,name="Genes", column_title="genes" )
		}
    }
    else{
