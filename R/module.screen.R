...
module.screen<-function(res.module, feature.patients=NULL, feature.genes=NULL, show.mods=NULL, show.n=4, method=c("ratio","fisher.test")[1], cores=1){
	...
	if(!is.null(feature.patients)){
		...
		pa.overlap=unlist(mclapply(used.mods, function(mod){
			...
			if(method=="ratio"){
				...
				p=sapply(1:length(c.feature), function(i){
					m=matrix(c(c.feature[i], c.total[i]-c.feature[i], xx, n.add.patients - xx),ncol=2, byrow=TRUE)
		...
		for(mod in sort.mods[1:show.n]){
			...
			plot(c(0, max(c.total)), c(0, length(feature.patients[feature.patients%in%add.patients])), xlab="Total patients",ylab="Observed Features",main="",type="n")
			points(c.total, c.feature)
			abline(b=length(feature.patients[feature.patients%in%add.patients])/num.patients, a=0,col="lightblue", lty=3, lwd=3)
