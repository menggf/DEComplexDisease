...
module.modeling<-function(res.module, keep.gene.num=NULL, model.method=c("slope.clustering", "max.square", "min.slope", "min.similarity")[1], cores=1, overlap=NULL, para=NULL){
	...
	mdl=mclapply(mods, function(mod){
		...
		newsc=apply(sub.deg,2, function(x) length(which(x== new.seed))/new.n)
